...
- **Schedule Planner blocks scheduling once a driver's license lapses (2026-09-16)**: a driver isn't supposed to be on the road once their license is expired, so the planner now locks a cell entirely - not just crossed out, actually unclickable - once it falls within one month of, or on/after, the driver's `LicenseExpiryDate`. New `ScheduleDayCell.IsLicenseIneligible` (computed per cell in `GetWeekScheduleAsync` via `DriverManagementService.IsLicenseIneligibleOn`) reuses the same 1-month cutoff as `IsEligibleForShift` (deliberately *not* `ComputeLicenseStatus`'s "Expiring" badge, which uses a 3-month window for display purposes only), but evaluates it **per calendar day** rather than just "now" - a manager who pages the planner forward to a future week sees a cell go ineligible exactly when that date reaches the 1-month-before-expiry mark, not only once "today" crosses it. An ineligible cell shows "🚫 INELIGIBLE" instead of its unit/day-off state and can't be toggled either direction (working → day off, or day off → working) - both `MarkDayOffAsync` and `ClearScheduleAsync` also re-check eligibility server-side (`EnsureLicenseEligibleOnAsync`, a fresh single-document read) rather than trusting the UI alone, since this is a real business rule, not just a UI nicety. A driver with no `LicenseExpiryDate` on file is never restricted by this rule.
